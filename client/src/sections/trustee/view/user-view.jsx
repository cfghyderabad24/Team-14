  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        setStudents(data.filter(student => student.approvedBy.includes('NGO') && student.approvedBy.includes('volunteer')));
        setLoading(false);
      } catch (error) {
  ...
  const handleApprove = async (id) => {
    try {
      const response = await fetch(`http://localhost:8000/api/scholarshipRequests/${id}`, {
        ...
        body: JSON.stringify({ approvedBy: ['volunteer','NGO','Trustee'] }),
      });
